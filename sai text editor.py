...
main_application.title('Sai Text Editer')
############################ main menu ##########################
main_menu=tk.Menu()
...
################################### theme commmand add  #################################################
theme_choice=tk.StringVar()  #theme varible
...
#bold button
def change_bold():
    text_property=tk.font.Font(font=text_editor['font'])   #it gives dictionary
...
def change_italic():
    ...
    if text_property.actual()['slant']=='italic':
        text_editor.configure(font=(cur_font_family,cur_font_size,'roman'))
...
def exit_func(event=None):
    ...
    try:
        if text_changed:
            mbox=messagebox.askyesnocancel('warning','Do yuo want to save the file ?')
            if mbox is True:
                if url:
                    content=text_editor.get(1.0,tk.END)
                    with open(url,'w',encoding='utf-8') as fw:
...
def hide_toolbar():
    global show_toolbar
    if show_toolbar:
        tool_bar.pack_forget()  #here we dide the toolbar
        show_toolbar=False
    else:
